...
<div class="container">
    <div class="raw head-top">
        <div class="col-md-3 text-center">
            <img src="../../${specialWorks.specialImage}" class="img-thumbnail" style="height: 130px"/>
        </div>
        <div class="col-md-9">
            <span class="up-top">
                <span style="float:left"><strong>${specialWorks.specialName}</strong></span>
                <button type="button" class="btn btn-default" style="float:right">
            <span class="glyphicon glyphicon-plus" aria-hidden="true">收藏</span></button></span>
            <span class="up-down">
               <ul style="list-style: none">
                   <li><a href="${pageContext.request.contextPath}/frontDesk/singerListById.do?id=${specialWorks.singerId}">${specialWorks.singernameChinese}</a></li>
                <li><fmt:formatDate value="${specialWorks.specialRelesetime}"></fmt:formatDate></li>
            </ul>
           </span>
        </div>
    </div>
...
<script>
    ...
    var specialId ='${specialWorks.specialId}';

    getSongList(currentPage, avageNum,specialId);
    initPagination(totalPage, totalNum,specialId);

    //与后台交互获取数据，异步加载到页面上
    function getSongList(pageNum, pageSize,specialId) {
        currentPage = pageNum;
        $.ajax({
            url: "/frontDesk/specialPage.do",
            method: "get",
            data: {"page": pageNum, "limit": pageSize,"specialId":specialId},
            async: false,
            success: function (data) {
                ...
                for (var i = 0; i < length; i++) {
                    ...
                    $(".songListTable tbody").append(
                        ...
                        '<td colspan="8"><a href="/musicPlayer/singerById.do?id='+singerId+'">'+singernameChinese+'</td>'+
                        '<td colspan="3"> ' + songLongTime+'</td>'+
                        '</tr>'
                    )
                }
    ...
    //初始化分页栏
    function initPagination(totalPage, totalNum,specialId) {
        ...
        for (var i = 1; i <= totalPage; i++) {
            $("#pagpaginationOneSinger ul").append(
                '<li id="page' + i + '"><a href="javascript:void(0);"  onclick="getSongList(' + i + ',' + avageNum +','+specialId+ ')">' + i + '</a>'
            )
        }
        $("#pagpaginationOneSinger ul").append(
    ...
    $("#pagpaginationOneSinger").on("click", "li", function () {
        ...
        if (aText == "上一页") {
            ...
            getSongList(currentPage - 1, avageNum,specialId);
            checkA();
        } else if (aText == "下一页") {
            $(".pagination > li").removeClass("active");
            $("#page" + (currentPage + 1)).addClass("active");
            ...
            $(".songListTable tbody").empty();
            getSongList(aText, avageNum,specialId);
            currentPage = aText - 1;
        }
    })
